<template>
	<view>
		<view id="top-box" class="cu-bar bg-white solid-bottom" style="background-color: #6739B6">
			<view style="display: none;" class="action text-black">
			...
			<uni-countdown background-color="#6739B6" color="#FFFFFF" splitor-color="#FFFFFF" :show-day="false" :hour="0"
			 :second="second" :reset="reset" @timeup="timeUp">
			</uni-countdown>
			<view class="action">
				<button style="display: none;" class="cu-btn bg-green shadow" @tap="showCardModal" data-target="modalCard">答题卡</button>
			</view>
		</view>
		...
		<form>
			<swiper :current="subjectIndex" class="swiper-box" @change="SwiperChange" :style="{'height':swiperHeight}">
				<swiper-item v-for="(subject,index) in subjectList" @touchmove.stop='stopTouchMove'>
		...
		<an-layer ref="anRef" autoClose="false" type="info"></an-layer>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			timeUp: function(e) { //倒计时结束事件
				this.$refs.anRef.show("倒计时结束，感谢您的参与", {
					type: 'error',
					autoClose: false
				});
			},
